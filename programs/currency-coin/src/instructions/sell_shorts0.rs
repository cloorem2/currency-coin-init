...
pub fn sell_shorts0(
    ...
    let mut b0_to_burn = cc_to_owner;
    b0_to_burn *= ctx.accounts.mint_authority.ccb_amount;
    b0_to_burn /= ctx.accounts.mint_authority.cc1_amount;
    b0_to_burn = b0_to_burn.floor();
    token::burn(
        CpiContext::new_with_signer(
            ctx.accounts.token_program.to_account_info(),
            token::Burn {
    ...
    cc_to_mint /= ctx.accounts.mint_authority.ccb_amount - b0_to_burn;
    ...
    ctx.accounts.mint_authority.cc0_amount += cc_to_mint;
    ctx.accounts.mint_authority.ccb_amount -= b0_to_burn;
