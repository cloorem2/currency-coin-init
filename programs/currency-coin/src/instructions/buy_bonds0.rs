...
pub fn buy_bonds0(
    ...
    let mut b0_to_owner = amount as f64;
    b0_to_owner += ctx.accounts.mint_authority.cc0_amount;
    b0_to_owner *= ctx.accounts.mint_authority.ccb_amount;
    b0_to_owner -= ctx.accounts.mint_authority.cc0_amount
      * ctx.accounts.mint_authority.ccb_amount;
    b0_to_owner /= ctx.accounts.mint_authority.cc0_amount + amount as f64;
    b0_to_owner = b0_to_owner.floor();
    token::transfer(
    ...
    let mut cc_from_owner = ctx.accounts.mint_authority.ccb_amount - b0_to_owner;
    cc_from_owner *= ctx.accounts.mint_authority.cc0_amount;
    cc_from_owner = ctx.accounts.mint_authority.cc0_amount
      * ctx.accounts.mint_authority.ccb_amount - cc_from_owner;
    cc_from_owner /= ctx.accounts.mint_authority.ccb_amount - b0_to_owner;
    ...
    let mut s0_to_mint = ctx.accounts.mint_authority.cc1_amount - b0_to_owner;
    s0_to_mint *= ctx.accounts.mint_authority.ccs_amount;
    s0_to_mint = ctx.accounts.mint_authority.ccs_amount
      * ctx.accounts.mint_authority.cc1_amount - s0_to_mint;
    s0_to_mint /= ctx.accounts.mint_authority.cc1_amount - b0_to_owner;
    ...
    ctx.accounts.mint_authority.ccb_amount -= b0_to_owner;
    ctx.accounts.mint_authority.cc1_amount -= b0_to_owner;
