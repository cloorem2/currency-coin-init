...
pub fn buy_shorts0(
    ...
    let mut b0_to_mint = cc_from_owner;
    b0_to_mint *= ctx.accounts.mint_authority.ccb_amount;
    b0_to_mint /= ctx.accounts.mint_authority.cc1_amount;
    b0_to_mint = b0_to_mint.floor();
    token::mint_to(
        CpiContext::new_with_signer(
            ctx.accounts.token_program.to_account_info(),
            token::MintTo {
    ...
    cc_to_burn /= ctx.accounts.mint_authority.ccb_amount + b0_to_mint;
    ...
    ctx.accounts.mint_authority.ccb_amount += b0_to_mint;
    Ok(())
}
